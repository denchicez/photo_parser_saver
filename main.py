...
def save_images(URL,name,PATH):
    try:
        ...
        if(URL.find('sunnydress')!=-1):
            soup = soup.find('div',class_='main-group region nested grid12-9')
        ...
        if(len(images_src_set)==0):
            errors.add(URL)
        for href in images_src_set:
            image_href = catalog+href
            try:
                resource = urllib.request.urlopen(image_href)
            except:
                continue
            if(len(images_src_set)==1):
                out = open(f"{PATH}/{name}.jpg", 'wb')
                out.write(resource.read())
                out.close()
            else:
                count+=1
                out = open(f"{PATH}/{name}_{count}.jpg", 'wb')
                out.write(resource.read())
                out.close()
        ...
        print(e)
        errors.add(URL)
URL =  "http://sunnydress.ru/futbolka-model-392.aspx"
urls = list()
names = list()
if __name__ == '__main__':
    ...
    print(hyperlinks[0].url_or_path)
